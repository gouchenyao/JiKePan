...
def init_db():
    with app.app_context():
        ...
        with open(app.root_path + '\\course_tables\\all_courses.txt', mode='r', encoding='utf-8') as html_all_courses:
            courses_all = timetable_all_parser(html_all_courses)
            for course_single in courses_all:
                db.execute(
                    'insert into courses (course_id, course_name, teacher_name ,course_schedule) values (?, ?, ?, ?)',
